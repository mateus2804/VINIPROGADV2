...
namespace XPTOMVC.Controllers
{
	public class PrestadorServicosController : Controller
	{
		...
		private readonly PrestadorServicoService _prestadorService;

		public PrestadorServicosController(PrestadorServicoService prestadorService)
		{
		...
		{
			var Servicos = await _prestadorService.FindAllAsync();
        ...
        public async Task<IActionResult> Create(PrestadorServico prestador) //Create - POST
        {
            ...
            try
            {
                ...
                {
                    await _prestadorService.CreateAsync(prestador);
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var req = await _prestadorService.FindByCPFAsync(id.Value);
            if (req == null)
            {
                return RedirectToAction(nameof(Error), new { Message = "Esse Prestador não está registrado!" });
            }
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public async Task<IActionResult> Edit(PrestadorServico prestador)// Edit - POST
		{
			try
			{
				await _prestadorService.UpdateAsync(prestador);
				return RedirectToAction(nameof(Index));
			}
        ...
        public async Task<IActionResult> Delete(int id) //Delete - POST
        {
            try
            {
                await _prestadorService.RemoveAsync(id);
                return RedirectToAction(nameof(Index));
            }
